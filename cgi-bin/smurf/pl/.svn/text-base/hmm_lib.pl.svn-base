...
#this subroutine need accurate lend and rend saved in sequence and header info in STOCKHOLM format
    ...
    for my $n ( 0 .. $$seq_r{number} - 1 ) {
        ...
        if (    $$seq_r{stockholm}{GF}{score}{$acc}{hmm} ne ''
            and $domain_num > 0
            and $$seq_r{stockholm}{GF}{score}{$acc}{$domain_num}{b} <
            $cutoff2
            and $cutoff2 =~ /\d+/ )
        {
            delete $$seq_r{$n};
...
#	    system "cat $dir_ind/${hmm_acc}_bin.HMM >> $dir_lib/PFAM_LIB_bin.HMM" if($hmm_acc =~ /^PF/);
        ...
        if ( -s "$dir_ind/$hmm_acc.FRAG" ) {
            ...
            system
              "cat $dir_ind/$hmm_acc.FRAG       >> $dir_lib/ALL_LIB.FRAG";
...
#             "cat $dir_ind/${hmm_acc}_bin.FRAG >> $dir_lib/ALL_LIB_bin.FRAG";
            system
              "cat $dir_ind/$hmm_acc.FRAG       >> $dir_lib/TOGA_LIB.FRAG"
              if ( $hmm_acc =~ /^TIGR/ );
# fix 2
            system
...
#             "cat $dir_ind/${hmm_acc}_bin.FRAG >> $dir_lib/TOGA_LIB_bin.FRAG"
              if ( $hmm_acc =~ /^TIGR/ );
            system
              "cat $dir_ind/$hmm_acc.FRAG       >> $dir_lib/PFAM_LIB.FRAG"
              if ( $hmm_acc =~ /^PF/ );
# fix 3
            system
...
# Thus:
    ...
    if ( $arch != /linux/i ) {
        die
          "Your operating system is impure: It does not support LDhmmer.\n";
    }
    $hmmconvert = "hmmconvert";
    if (   $family eq 'TIGR'
        || $family =~ /^ALL$/i )
    {
        ...
        while (<$dir_ind/TIGR*HMM>) {
            system "cat $_ >> $dir_lib/TOGA_LIB.HMM"
              ;  # if($_ =~ /^TIGR\d{5}\.HMM$/);
        }
...
# 	system "hmmindex $dir_lib/TOGA_LIB_bin.FRAG > $dir_lib/TOGA_LIB_bin.FRAG.gsi.log" if(-s "$dir_lib/TOGA_LIB_bin.FRAG");
        ...
        || $family =~ /^ALL$/i )
    {
        ...
        while (<$dir_ind/PF*HMM>) {
            system "cat $_ >> $dir_lib/PFAM_LIB.HMM"
              ;  # if($_ =~ /^PF\d{5}\.HMM$/);
        }
        while (<$dir_ind/PF*FRAG>) {
            system "cat $_ >> $dir_lib/PFAM_LIB.FRAG"
              ;  # if($_ =~ /^PF\d{5}\.FRAG$/);
        }
        system
...
sub load_hmm {
        ...
        && $$in_r{trusted_cutoff2} !~ /^(-?\d+\.?\d*|\d*\.\d+)$/ )
    {
        $check = "error";
        $$log_r .=
          "***Error: $log_header--wrong format for trusted_cutoff2\n"
          if ($log_r);
    }
    ...
    my $hmm_gf = '';
    if ( $$in_r{hmm} =~ /\S/ ) {
        &get_hmm_glo_frac( $$in_r{hmm}, \$hmm_gf, $$in_r{hmm_acc},
            $log_header, $log_r );
    }
    ...
    if ( $$in_r{hmm_id} !~ /^\d+$/ ) {
        ...
        my %data;
        &get_INFO_from_db( $db_proc, $db, $$in_r{hmm_acc}, "hmm_acc",
            \%data );
...
#	$query_hmm2 .= "struct_acc=\"$$in_r{struct_acc}\", " if($$in_r{struct_acc} ne $data{struct_acc});
    ...
    &convert_null( '', 'null', \$$in_r{seed_ref_id},
        \$$in_r{seed_user_class} );
    if ( @$ref == 0 ) {
...
sub get_status_vocabulary {
    ...
    for (@$ref) {
        $$status_r{ $$_{table_name} }{ $$_{field_name} }
          { $$_{status_type} } = $$_{id};
    }
}
    ...
    for my $n (@$data_r) {
        ...
        for my $m (@new_data) {
            my ( $new_lend, $new_rend ) =
              &overlap_fragment( $$n{lend}, $$n{rend}, $$m{lend},
                $$m{rend} );
            if ( $new_lend ne '' ) {
...
# new keys under $$prot_r{$acc} are: name, genus, species, length, seq, other_acc, first_acc
    use strict;
    my ( $prot_r, $log_header, $log_r, $prog, $cdbyank_index_file, $dir, $file_root ) = @_;
        ...
        for my $list (@yank_list) {
            ...
            if (    $dir ne '' and $file_root ne '' )
            {
                ...
                if (   $prog eq "yank" or $prog eq '' ) {
                    ...
                    system "echo $list | cdbyank $cdbyank_index_file > $dir/$tmp_file";
                }
...
#	    &print_cyc("/usr/local/projects/OGC/egad_test/ogc_cyc_log/$file", "BB4 yank\n");
                ...
                if (   $prog eq "yank"
                    or $prog eq '' )
                {
                    open( FH, "echo $list | /usr/local/packages/EMBOSS/bin/yank |" );
                }
                elsif ( $prog eq "cdbyank" ) {
                    open( FH,
...
sub yank_on {
    ...
    {
        no strict "refs";
        open( $handle, "/usr/local/packages/EMBOSS/bin/yank -t -a \"EGAD\|11\" | " );
        while (<$handle>) {
            $result .= $_;
        }
        close $handle;
...
#If $clean_time is set, any directory named $new_dir with a date older than that time from the current time will be deleted.
    ...
    my ( $mode, $base_dir, $new_dir, $created_dir_r, $clean_time, $log_r ) =
      @_;
    my $curr_dir = cwd();
...
#note that hmm2 table is not involved yet
    ...
    &do_query( $db_proc, "delete $db..status where project_id = $proj_id",
        1 );
    &do_query( $db_proc,
        "delete $db..alignment where iso_id in ($iso_id_list)", 1 );
    &do_query( $db_proc,
        "delete $db..isology where iso_id in ($iso_id_list)", 1 );
    &do_query( $db_proc,
        "delete $db..iso_link where iso_id in ($iso_id_list)", 1 );
    &do_query( $db_proc,
        "delete $db..project_iso_link where project_id = $proj_id", 1 );
    &do_query( $db_proc,
        "delete $db..iso_project where project_id = $proj_id", 1 );
...
sub process_check_hmm_data {
    use strict;
    my ( $db, $db_proc, $class, $data_r, $name, $fatal_r, $LOG1, $LOG2 ) =
      @_;
    my ( @HMM, @FRAG );
    @HMM = split( /\n/, $$data_r{hmm}{value} )
      if ( $$data_r{hmm}{value} );
...
# check hmm_acc.  hmm_acc might be found in INFO stored in $data{hmm_acc}{value}, HMM, FRAG, or as the file name for PFAM
    ...
    for my $s1 ( $$data_r{hmm_acc}{value}, $HMM_acc, $FRAG_acc, $file_acc ) {
        for
          my $s2 ( $$data_r{hmm_acc}{value}, $HMM_acc, $FRAG_acc, $file_acc )
        {
            if (   $s1 =~ /\S/
                && $s2 =~ /\S/
                && $s1 ne $s2 )
...
#check hmm_name that might be found in field ID in INFO stored in $data{hmm_name}{value}, field NAME in PFAM HMM, field NAME in PFAM FRAG, and file name for TOGA
    ...
    $file_name = $name
      if ( $class eq 'TOGA' );
    for
      my $s1 ( $$data_r{hmm_name}{value}, $HMM_name, $FRAG_name, $file_name )
    {
...
sub trim_seq {
    ...
    for my $n ( 1 .. $$seq_r{number} - 1 ) {
        $$log_r .=
          ...
          if (
            length( $$seq_r{$n}{seq_gap} ) !=
            length( $$seq_r{0}{seq_gap} ) );
    }
...
# LOG2: log file for long output
    use strict;
    my ( $seq_r, $format, $seq_num, $length, $gap, $header, $LOG1, $LOG2 ) =
      @_;
    my $index = 0;
    for my $n ( 0 .. $$seq_r{number} - 1 ) {
        if ( defined $$seq_r{$n}
    ...
    if ( $format eq "mul" ) {
        ...
        if ( $$seq_r{stockholm} ne '' ) {
            ...
            for my $n (
                0 .. $$seq_r{stockholm}{GF}{hmmbuild_method}{number} - 1 )
            {
                $result .=
...
sub check_data {
        ...
        && $$data_r{align_seed}{ori} )
    {
        print $LOG2
          ...
          ;
        &check_MA_MA( $$data_r{hmm_seed}, $$data_r{align_seed}, 'eq',
        ...
        && $$data_r{align_full}{ori} )
    {
        print $LOG2
          ...
          ;
        &check_MA_MA( $$data_r{hmm_seed}, $$data_r{align_full}, 'le',
            $fatal_r, $$data_r{header}, $LOG1, $LOG2 );
    }
    if (   $$data_r{iso_link}
        && $$data_r{align_full}{ori} )
    {
        ...
        &check_lend_rend_in_iso_link( $$data_r{align_full},
            $$data_r{iso_link}, $$data_r{iso_id}, \$$flag_r{iso_link},
            $fatal_r, $$data_r{header}, $LOG1, $LOG2 );
    }
}
...
sub check_lend_rend_in_iso_link {
    use strict;
    my ( $ma_r, $iso_link_r, $iso_id, $flag_r, $fatal_r, $header, $LOG1,
        $LOG2 )
      = @_;
    my @new_iso_link;
    ...
    foreach my $k ( 0 .. $$ma_r{number} - 1 ) {
        if ( $$ma_r{$k}{header} =~ /^([^\/\|\s]+)\|([^\/\|\s]+)/ ) {
            ...
            if ( $$ma_r{stockholm}{GF}{score}{"EGAD|$$ma_r{$k}{prot_id}"} ) {
                $$iso_link_r[$m]{score} =
                  $$ma_r{stockholm}{GF}{score}{"EGAD|$$ma_r{$k}{prot_id}"}
                  {b};
                $$iso_link_r[$m]{e_val} =
                  $$ma_r{stockholm}{GF}{score}{"EGAD|$$ma_r{$k}{prot_id}"}
                  {e};
...
sub load_iso_link {
    ...
    my ( $revise, $iso_id, $iso_link_ref, $db, $db_proc, $LOG, $DB_LOG,
        $header )
      = @_;
    &do_query( $db_proc, "delete from $db..iso_link where iso_id = $iso_id",
        1 )
      if ($revise);
    for (@$iso_link_ref) {
    ...
    foreach my $k1 ( 0 .. $$ma1_r{number} - 1 ) {
        ...
        foreach my $k2 ( 0 .. $$ma2_r{number} - 1 ) {
            ...
            if ( $s2 ne '' ) {
                ...
                print $LOG1
                  "SEQ_ERROR\t$header\tempty sequence $k2 in MA2\n" ;
                print $LOG2
                  "SEQ_ERROR\t$header\tempty sequence $k2 in MA2\n" ;
            }
        }
...
sub recursive_match {
    ...
    if ( index( $a, $c ) >= 0 ) {
        ...
        my ( $success2, $c2 ) =
          &recursive_match( $a, $b, substr( $c, $half ) );
        if (   $success1
            && $success2
            && length($c1) <= length($c2) )
        {
...
# check sequences in MA with full length protein sequence, and calculate lend and rend
        if ( $$ma_r{$k}{prot_id} ne '' ) {
            ...
            if ($success) {
                ...
                my $success =
                  &search_nraa( $$ma_r{$k}, $flag_r, $fatal_r, $header,
                    $LOG1, $LOG2 )
                  if ($blastp);
                if ($success) {
                    print $LOG1
...
sub run_blastp {
    ...
    my $line =
      &format_sequence( $seq_r, 'fasta', $seq_num, 60, 0, '', '', '' );
    $tmp_root .= '_';
...
sub make_seed_from_blastp_hits {
    ...
    for my $prot_acc ( keys %prot_acc ) {
        ...
        $$log_r .=
          "***Error: $log_header--can not cdbyank protein $prot_acc\n"
          if (  $log_r
            and $$seed_r{$seq_name}{ori} !~ /\S/ );
    }
    ...
    foreach my $k ( 0 .. $$ma_r{number} - 1 ) {
            ...
            && $$ma_r{$k}{locus} =~ /-/ )
        {
            ...
            print $LOG2
              ...
              ;
            print $LOG1
              "HEADER_UPDATE\t$header\tUpdating dash to underscore\n" ;
            print $LOG2
              "HEADER_UPDATE\t$header\tUpdating dash to underscore\n" ;
            $$ma_r{$k}{locus} =~ s/-/_/g;
            $$flag_r .=
...
sub check_MA_content {
    ...
    for my $k ( 0 .. $$ma_r{number} - 1 ) {
        ...
        if ( $$ma_r{$k}{seq_gap} eq '' ) {
            print $LOG1
              "SEQ_ERROR\t$header\tempty sequence \#$k (with gap)\n"
              if ($LOG1);
            print $LOG2
              "SEQ_ERROR\t$header\tempty sequence \#$k (with gap)\n"
              if ($LOG2);
...
sub process_data {
    ...
    &parse_sequence( \%{ $data_r->{hmm_seed} },
        "mul", \$$flag_r{hmm_seed}, $fatal_r, $$data_r{header}, $LOG1,
        $LOG2 )
      if ( $$data_r{hmm_seed}{ori} );
    ...
    if ( $format eq "mul" ) {
        ...
        for my $n (@li) {
            if ( $li[0] =~ /^\#\s*STOCKHOLM/ ) {
                ...
                {
                    ++$count_mini_db;
                    $$seq_r{stockholm}{GF}{mini_db}{$count_mini_db}{acc} =
                      $1;
                    $$seq_r{stockholm}{GF}{mini_db}{$count_mini_db}{seq} =
                      $2;
                    $$seq_r{stockholm}{GF}{mini_db}{$count_mini_db}{cmd} =
                      $3;
                    $$seq_r{stockholm}{GF}{mini_db}{$count_mini_db}
                      {db_name} = $4;
                    $$seq_r{stockholm}{GF}{mini_db}{$count_mini_db}
                      {db_size} = $5;
                    $$seq_r{stockholm}{GF}{mini_db}{$count_mini_db}
...
# get_hmm_seed_from_db must be run first to get iso_id and hmm_acc and compose $header.
    ...
    &get_hmm_seed_from_db( $info_r, $data_r, $flag_r, $fatal_r, $LOG1,
        $LOG2 );
    print $LOG2 "get alignment_seed from db.......................\n" ;
    &get_alignment_from_db( $info_r, $data_r, "seed", $flag_r, $fatal_r,
        $LOG1, $LOG2 );
    print $LOG2 "get alignment_full from db.......................\n" ;
    &get_alignment_from_db( $info_r, $data_r, "all", $flag_r, $fatal_r,
        $LOG1, $LOG2 );
    print $LOG2 "get iso_link from db.....................\n" ;
    &get_iso_link_from_db( $info_r, $data_r, $LOG1, $LOG2 );
    print $LOG2
      "get full length protein sequence from db..................\n" ;
    &get_protein_from_db( $$info_r{db}, $$info_r{db_proc}, "iso_link",
        "iso_id", $data_r, $LOG1, $LOG2 );
}
...
# $sleep: -1: die,  0: return 1, > 0: sleep seconds.
    ...
    while ( !$$info_r{db_proc}->{Active} ) {
        print
          "${\(scalar localtime)}: disconnected when processing $query\n";
...
#    print LOG "\t\t$accession, $seqF, $seqT, $hmmF, $hmmT, $total_score, $trusted, $noise, $expect_whole\n";
    if ($SEE) {
        ...
        print
          "Expect value (domain/total): $expect_domain/$expect_whole\n\n";
    }
...
#	    print LOG "\t\tERROR: $accession hits $feat_name more than once at the same coords.\n";
        ...
        if ( @newhits == 1 ) {
            ...
            if ( length($fragment) < 1 ) {
                print
                  "$feat_name, $asmbl_id:$length, $EVend5, $EVend3\n\n\n";
                exit();
            }
